...
try {
    ...
    // When the JAR is built by enumerating individual files, the `jar` tool omits the intermediate
    // directory entries, which breaks consumers relying on JAR directory traversal
    // (e.g. Spring Boot @ComponentScan). Assert they are present in BOTH the base area and the
    // versioned (META-INF/versions/17) area.
