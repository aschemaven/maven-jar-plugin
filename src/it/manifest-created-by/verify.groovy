...
try {
    ...
    // The jar tool's fallback value, which we must NOT see.
    String jarToolValue = System.getProperty("java.version") + " (" + System.getProperty("java.vendor") + ")"

    assert createdBy != null : "Created-By is missing; the jar tool likely omitted it"
    assert createdBy != jarToolValue : "Created-By is the jar tool's JDK-specific value: " + createdBy
    // A JDK-independent value: our plugin identity, never the "<version> (<vendor>)" shape.
    assert createdBy.startsWith("Maven JAR Plugin") : "Unexpected Created-By: " + createdBy
    assert !createdBy.contains("(") : "Created-By looks JDK-specific: " + createdBy
